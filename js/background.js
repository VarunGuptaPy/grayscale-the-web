...
chrome.runtime.onInstalled.addListener(function (details) {
  ...
  if (details.reason == "install" || details.reason == "update") {
    ...
    chrome.storage.sync.get(
      ["gsSites", "gsExcluded", "gsAll", "gsTabs", "gsBgToggle"],
      function (val) {
...
// Clear out gsTabs and temporary ungrayscale tabs when chrome is closed
chrome.storage.sync.set({ gsTabs: [] });
chrome.storage.local.set({ gsTempUngrayscale: [] });

// Add tab removal listener
chrome.tabs.onRemoved.addListener(function (tabId) {
  // Clean up temporary ungrayscale state when tab is closed
  chrome.storage.local.get(["gsTempUngrayscale"], function (val) {
    if (val.gsTempUngrayscale) {
      const tempUngrayscale = val.gsTempUngrayscale.filter(
        (id) => id !== tabId
      );
      chrome.storage.local.set({ gsTempUngrayscale: tempUngrayscale });
    }
  });
});
  ...
  chrome.storage.sync.get(
    ...
    function (val) {
      ...
      chrome.storage.local.get(["gsTempUngrayscale"], function (tempVal) {
        ...
        // If tab is temporarily ungrayscaled, skip grayscale
        if (tempUngrayscale.includes(info.tabId)) {
          console.log("tab is temporarily ungrayscaled");
          chrome.tabs.sendMessage(info.tabId, { type: "turnOffGray" });
          turnIconOff();
          return;
        }
